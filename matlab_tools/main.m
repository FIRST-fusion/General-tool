function main(inputfile,coilfile) %varargin coils
...
% phi=0:2*pi/(359):0.67*2*pi;     % 361 points define phi [0,6.28]

r=cfunct(theta,phi,rmnc,xm,xn);  % Cos transform
z=sfunct(theta,phi,zmns,xm,xn);  % Sin transform
modb=cfunct(theta,phi,bmnc,xm_nyq,xn_nyq);  % Cos Transform
